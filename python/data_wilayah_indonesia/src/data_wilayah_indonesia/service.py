...
# Default bundled data path
DEFAULT_DATA_PATH = os.path.join(os.path.dirname(__file__), "data", "indonesia_administrative_data.csv")
...
class DataWilayahService:
    ...
    Args:
        data_path: Path ke file CSV data wilayah.
                   Jika tidak diberikan, akan menggunakan data bundled.
    """
    def __init__(self, data_path: Optional[str] = None):
        # Use bundled data if no path provided
        actual_path = data_path if data_path else DEFAULT_DATA_PATH
        
        # Inisialisasi Repositories (Infrastructure Layer)
        self.province_repo = CSVProvinceRepository(actual_path)
        self.regency_repo = CSVRegencyRepository(actual_path)
        self.district_repo = CSVDistrictRepository(actual_path)
        self.village_repo = CSVVillageRepository(actual_path)
